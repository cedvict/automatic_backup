...
from openerp import service,tools, _
...
addons_path = tools.config['addons_path'] + '/automatic_backup/DBbackups'
    ...
    def schedule_backup(self, cr, user, context={}):
        ...
        for rec in confs:
            ...
            if rec.name in db_list:
                ...
                try:
                    dump_stream = openerp.service.db.dump_db(rec.name, None, rec.backup_type)
                except:
                    _logger.debug(
                ...
                try:
                    ...
                    for file in srv.listdir(pathToWriteTo):
                        ...
                        if delta.days >= rec.daystokeepsftp:
                            ...
                            if srv.isfile(fullpath) and (".dump" or ".zip") in file:
                    ...
                    if rec.sendmailsftpfail:
                        try:
                            ...
                            message = "Dear,\n\nThe backup for the server " + rec.host + " (IP: " + rec.sftpip + ") failed.Please check the following details:\n\nIP address SFTP server: " + rec.sftpip + "\nUsername: " + rec.sftpusername + "\nPassword: " + rec.sftppassword + "\n\nError details: " + tools.ustr(
                                e) + "\n\nWith kind regards"
                            msg = ir_mail_server.build_email("automatic_backup@" + rec.name + ".com", [rec.emailtonotify],
            ...
            if rec.autoremove is True:
                ...
                for f in os.listdir(dir):
                    ...
                    if delta.days >= rec.daystokeep:
                        ...
                        print 'Autoback start %s' % os.path.isfile(fullpath)
